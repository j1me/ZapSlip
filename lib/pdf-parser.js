...
function extractItems(text) {
    ...
    while (i < lines.length) {
        ...
        // Check if the product name line already contains quantity information
        const quantityInNameMatch = productName.match(/^(.+?)\s+(\d+\s+of\s+\d+)$/);
        if (quantityInNameMatch) {
            productName = quantityInNameMatch[1].trim();
            const qtyMatch = quantityInNameMatch[2].match(/^(\d+)\s+of\s+\d+$/);
            if (qtyMatch) {
                quantity = parseInt(qtyMatch[1], 10);
                console.log(`  → Extracted quantity from product name: "${productName}", qty: ${quantity}`);
                // Skip processing since we have everything we need
                if (productName.trim()) {
